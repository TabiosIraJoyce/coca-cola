<x-app-layout>
    ...
    <div class="p-6 space-y-6 print-area">
        ...
        <form id="periodForm"
            ...
            @csrf
            {{-- Ensure these are parsed before large tables (avoid max_input_vars issues) --}}
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="branch"
                            ...
                            <option value="Solsona" {{ $branchValue === 'Solsona' ? 'selected' : '' }}>Solsona</option>
                            <option value="Laoag" {{ $branchValue === 'Laoag' ? 'selected' : '' }}>Laoag</option>
                            <option value="Batac" {{ $branchValue === 'Batac' ? 'selected' : '' }}>Batac</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="period_no" id="period_no"
                            ...
                            @for($i = 1; $i <= 12; $i++)
                                <option value="{{ $i }}"
                                    {{ (string)$periodNoValue === (string)$i ? 'selected' : '' }}>
                                    Period {{ $i }}
                                </option>
                            @endfor
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date"
                            ...
                            value="{{ old('report_date', $date ?? now()->toDateString()) }}"
                            required>
                    </div>
                </div>

                @if(empty($isEdit) && (($existingReportCount ?? 0) > 0))
                    <div class="mt-3 p-3 rounded-lg border border-amber-200 bg-amber-50 text-sm text-amber-900 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                        <div>
                            {{ ($existingReportCount ?? 0) > 1 ? 'Multiple reports already exist' : 'A report already exists' }}
                            for this Branch / Period / Date.
                            You can still create another report for a different <span class="font-semibold">Shipment No</span>.
                        </div>
                        @if(!empty($existingReportId))
                            <a href="{{ route('admin.reports.periods.edit', $existingReportId) }}"
        ...
        {{-- =========================
    CATEGORY TARGETS (MEDIUM KPI BOXES)
========================= --}}
    ...
    <div class="border rounded-lg p-4 bg-red-50">
        <p class="text-xs font-bold text-red-700 uppercase mb-2">
            Core Target
             ...
             value="{{ number_format($coreTarget, 2) }}"
             readonly>


         <div class="flex justify-between text-xs text-red-700 mt-3">
             <span>Actual</span>
             <span id="core_actual" class="font-semibold">{{ number_format($report?->core_actual_sales ?? 0, 2) }}</span>
         </div>
  
         <div class="flex justify-between text-xs text-red-700 mt-1">
             <span>Variance</span>
             <span id="core_variance" class="font-semibold">{{ number_format($report?->core_variance ?? 0, 2) }}</span>
         </div>

        <div class="text-xs text-red-700 mt-1 text-right">
            Achievement:
            <span id="core_achievement" class="font-semibold">{{ number_format($report?->core_achievement_pct ?? 0, 2) }}%</span>
        </div>
    </div>
    ...
    <div class="border rounded-lg p-4 bg-yellow-50">
        <p class="text-xs font-bold text-yellow-700 uppercase mb-2">
         ...
         <input type="text"
             id="petcsd_target"
             class="w-full bg-transparent text-right text-lg font-bold"
             value="{{ number_format($petcsdTarget, 2) }}"
             readonly>

         <div class="flex justify-between text-xs text-yellow-700 mt-3">
             <span>Actual</span>
             <span id="petcsd_actual" class="font-semibold">{{ number_format($report?->petcsd_actual_sales ?? 0, 2) }}</span>
         </div>
  
         <div class="flex justify-between text-xs text-yellow-700 mt-1">
             <span>Variance</span>
             <span id="petcsd_variance" class="font-semibold">{{ number_format($report?->petcsd_variance ?? 0, 2) }}</span>
         </div>

        <div class="text-xs text-yellow-700 mt-1 text-right">
            Achievement:
            <span id="petcsd_achievement" class="font-semibold">{{ number_format($report?->petcsd_achievement_pct ?? 0, 2) }}%</span>
        </div>
    </div>
    ...
    <div class="border rounded-lg p-4 bg-blue-50">
        <p class="text-xs font-bold text-blue-700 uppercase mb-2">
         ...
         <input type="text"
             id="stills_target"
             class="w-full bg-transparent text-right text-lg font-bold"
             value="{{ number_format($stillsTarget, 2) }}"
             readonly>


         <div class="flex justify-between text-xs text-blue-700 mt-3">
             <span>Actual</span>
             <span id="stills_actual" class="font-semibold">{{ number_format($report?->stills_actual_sales ?? 0, 2) }}</span>
         </div>
  
         <div class="flex justify-between text-xs text-blue-700 mt-1">
             <span>Variance</span>
             <span id="stills_variance" class="font-semibold">{{ number_format($report?->stills_variance ?? 0, 2) }}</span>
         </div>

        <div class="text-xs text-blue-700 mt-1 text-right">
            Achievement:
            <span id="stills_achievement" class="font-semibold">{{ number_format($report?->stills_achievement_pct ?? 0, 2) }}%</span>
        </div>
    </div>
 ...
 >
    ...
    <div class="border rounded-lg p-4 bg-orange-50">
        ...
        <input type="hidden"
            ...
            {{-- Products table stays visible for daily encoding.
                 "Add report" always starts at zero unless you are explicitly editing an existing report. --}}

            {{-- =========================================
                 DETAIL TABLES / CUSTOM TABLES
            ========================================== --}}
                {{-- =========================
                    CORE + IWS DETAIL TABLE (PER SKU)
                ========================== --}}
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value="{{ old('shipment_no', $report->shipment_no ?? '') }}"
                    ...
                    <div class="overflow-auto mt-3">
                        <table class="min-w-full text-xs border border-gray-200 rounded-lg overflow-hidden"
                            ...
                            <tbody id="coreIwsBody" class="bg-white">
                                {{-- JS WILL RENDER ALL PRODUCT ROWS --}}
                            </tbody>
                ...
                <div class="mt-6">
                    ...
                    <div class="border rounded-lg bg-white shadow-sm overflow-x-auto">
                        <table class="min-w-full text-[11px] md:text-xs border" id="perSkuTable">
                            ...
                            <tbody id="perSkuBody">
                                {{-- Rows are injected via JS using PRODUCTS --}}
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="text-right mt-6">
                    <button type="submit"
                        ...
                        {{ isset($report) ? 'Update Report' : 'Save Full Report' }}
                    </button>
...
<script>
    ...
    const SAVED_INVENTORY = {!! json_encode(
        isset($inventoryRows)
    ...
    const SAVED_CUSTOM_TABLES = {!! json_encode($customTables ?? []) !!};

    const SAVED_PER_SKU = {!! json_encode(
    ...
    const IS_VIEW = {{ isset($isView) && $isView ? 'true' : 'false' }};
