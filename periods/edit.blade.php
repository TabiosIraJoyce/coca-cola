...
<x-slot name="header">
    <h2 class="text-xl font-bold">
        Edit Period {{ $report->period_no }} - {{ $report->branch }}
    </h2>
</x-slot>

<form id="periodEditForm" method="POST" action="{{ route('admin.reports.periods.update', $report->id) }}">
@csrf
@method('PUT')
...
<div class="bg-white p-4 rounded shadow grid grid-cols-4 gap-4">
    <div>
        ...
        <input type="text"
               id="target_sales"
               readonly
               value="&#8369;{{ number_format($report->target_sales,2) }}"
    ...
    <div>
        ...
        <input type="text"
               readonly
               value="&#8369;{{ number_format($report->core_target_sales ?? 0,2) }}"
               class="w-full bg-gray-100 border rounded px-2 py-1 text-right">
    </div>
    <div>
        ...
        <input type="text"
               readonly
               value="&#8369;{{ number_format($report->petcsd_target_sales ?? 0,2) }}"
               class="w-full bg-gray-100 border rounded px-2 py-1 text-right">
    </div>
    <div>
        ...
        <input type="text"
               readonly
               value="&#8369;{{ number_format($report->stills_target_sales ?? 0,2) }}"
               class="w-full bg-gray-100 border rounded px-2 py-1 text-right">
    </div>
    ...
    <div>
        ...
        <input type="text"
               id="actual_sales"
               readonly
               value="&#8369;{{ number_format($report->actual_sales,2) }}"
               class="w-full bg-gray-100 border rounded px-2 py-1 text-right">
    </div>
    ...
    <div>
        ...
        <input type="text"
               id="variance_display"
               readonly
               value="&#8369;{{ number_format($report->total_variance,2) }}"
               class="w-full bg-gray-100 border rounded px-2 py-1 text-right">
    ...
    <div>
        ...
        <input type="text"
               ...
               readonly
               value="{{ number_format($report->achievement_pct,2) }}"
               class="w-full bg-gray-100 border rounded px-2 py-1 text-right">

        <input type="hidden"
               id="achievement_pct"
               name="achievement_pct"
               value="{{ $report->achievement_pct }}">
    </div>
</div>
    ...
    <table class="w-full border text-xs">
        ...
        @foreach($report->items as $i => $item)
            ...
            <tr>
                <td class="border p-2">
                    {{ $item->pack }}
                    <input type="hidden" name="items[{{ $i }}][pack]" value="{{ $item->pack }}">
                </td>
...
<div class="bg-white shadow rounded p-4">
    ...
    <div class="overflow-auto">
        <table class="min-w-full border text-xs">
            ...
            <tbody>
            @forelse($report->items as $i => $item)
                @php
                ...
                <tr class="hover:bg-gray-50">
                    <td class="border p-2 whitespace-nowrap">
                        {{ $item->pack }}
                        <input type="hidden" name="inventories[{{ $i }}][pack]" value="{{ $item->pack }}">
                    </td>
                    <td class="border p-2 whitespace-nowrap">
                        {{ $item->product }}
                        <input type="hidden" name="inventories[{{ $i }}][product]" value="{{ $item->product }}">
                    </td>
                    <td class="border p-2">
                        <input type="number" step="0.01" min="0"
                               name="inventories[{{ $i }}][srp]"
                               value="{{ $inv->srp ?? 0 }}"
                               class="w-28 border rounded px-2 py-1 text-right">
                    </td>
                    <td class="border p-2">
                        <input type="number" step="0.01" min="0"
                               name="inventories[{{ $i }}][actual_inv]"
                               value="{{ $inv->actual_inv ?? 0 }}"
                               class="w-24 border rounded px-2 py-1 text-right">
                    </td>
                    <td class="border p-2">
                        <input type="number" step="0.01" min="0"
                               name="inventories[{{ $i }}][ads]"
                               value="{{ $inv->ads ?? 0 }}"
                               class="w-24 border rounded px-2 py-1 text-right">
                    </td>
                    <td class="border p-2">
                        <input type="number" step="0.01" min="0"
                               name="inventories[{{ $i }}][booking]"
                               value="{{ $inv->booking ?? 0 }}"
                               class="w-24 border rounded px-2 py-1 text-right">
                    </td>
                    <td class="border p-2">
                        <input type="number" step="0.01" min="0"
                               name="inventories[{{ $i }}][deliveries]"
                               value="{{ $inv->deliveries ?? 0 }}"
                               class="w-24 border rounded px-2 py-1 text-right">
                    </td>
                    <td class="border p-2">
                        <input type="number" step="0.01" min="0"
                               name="inventories[{{ $i }}][routing_days_p5]"
                               value="{{ $inv->routing_days_p5 ?? 0 }}"
                               class="w-24 border rounded px-2 py-1 text-right">
                    </td>
                    <td class="border p-2">
                        <input type="number" step="0.01" min="0"
                               name="inventories[{{ $i }}][routing_days_7]"
                               value="{{ $inv->routing_days_7 ?? 0 }}"
                               class="w-24 border rounded px-2 py-1 text-right">
                    </td>
                </tr>
            @empty
...
<div class="flex justify-end gap-3">
    <a href="{{ route('admin.reports.periods.show',$report->id) }}"
...
const SAVED_CUSTOM_TABLES = @json($customTables ?? []);
