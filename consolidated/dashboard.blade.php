...
<div class="flex gap-8 border-b mb-8 text-lg font-semibold">
    <a href="{{ route('admin.consolidated.index') }}"
       class="pb-3 {{ request()->routeIs('admin.consolidated.index')
    ...
    <a href="{{ route('admin.reports.periods.index') }}"
       class="pb-3 {{ request()->is('admin/reports/periods*')
            ? 'text-red-600 border-b-4 border-red-600'
            : 'text-gray-500 hover:text-red-600' }}">
        Period Summary
    </a>
...
<div class="bg-[#FFF6E5] rounded-xl shadow p-6 mb-8 border border-[#FFA69E]/40">
    <form method="GET"
          action="{{ route('admin.consolidated.index') }}"
          class="grid grid-cols-1 md:grid-cols-4 gap-4">

        <div>
            <label class="text-sm font-semibold">Branch</label>
            <select name="branch" class="w-full border rounded p-2">
                <option value="All" {{ request('branch', $branch) === 'All' ? 'selected' : '' }}>All</option>
                <option value="Solsona" {{ request('branch', $branch) === 'Solsona' ? 'selected' : '' }}>Solsona</option>
                <option value="Laoag" {{ request('branch', $branch) === 'Laoag' ? 'selected' : '' }}>Laoag</option>
                <option value="Batac" {{ request('branch', $branch) === 'Batac' ? 'selected' : '' }}>Batac</option>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="period_from" class="w-full border rounded p-2">
                @for($i = 1; $i <= 12; $i++)
                    <option value="{{ $i }}"
                        {{ request('period_from', $currentCutoff->period_no ?? $currentCutoff->period_number ?? now()->month) == $i ? 'selected' : '' }}>
                        Period {{ $i }}
                    </option>
                @endfor
            </select>
        ...
        <div>
            ...
            <select name="period_to" class="w-full border rounded p-2">
                @for($i = 1; $i <= 12; $i++)
                    <option value="{{ $i }}"
                        {{ request('period_to', $currentCutoff->period_no ?? $currentCutoff->period_number ?? now()->month) == $i ? 'selected' : '' }}>
                        Period {{ $i }}
                    </option>
                @endfor
            </select>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="space-y-1 text-sm">
            <p>Actual:
                <span class="font-bold">
                    {{ number_format($coreActual, 2) }}
                </span>
            </p>
            <p>Target:
                <span class="font-bold">
                    {{ number_format($coreTarget, 2) }}
                </span>
            </p>
            <p>Achievement:
                <span class="font-bold">
                    {{ number_format($coreAchievement, 2) }}%
                </span>
            </p>
            <p class="font-bold {{ $coreVariance < 0 ? 'text-red-600' : 'text-green-600' }}">
                Variance:
                {{ $coreVariance < 0 ? '-' : '+' }}
                {{ number_format(abs($coreVariance), 2) }}
            </p>
        </div>
    </div>
    ...
    <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 shadow">
        ...
        <div class="space-y-1 text-sm">
            <p>Actual:
                <span class="font-bold">
                    {{ number_format($petcsdActual, 2) }}
                </span>
            </p>
            <p>Target:
                <span class="font-bold">
                    {{ number_format($petcsdTarget, 2) }}
                </span>
            </p>
            <p>Achievement:
                <span class="font-bold">
                    {{ number_format($petcsdAchievement, 2) }}%
                </span>
            </p>
            <p class="font-bold {{ $petcsdVariance < 0 ? 'text-red-600' : 'text-green-600' }}">
                Variance:
                {{ $petcsdVariance < 0 ? '-' : '+' }}
                {{ number_format(abs($petcsdVariance), 2) }}
            </p>
        </div>
    </div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-6 shadow">
        ...
        <div class="space-y-1 text-sm">
            <p>Actual:
                <span class="font-bold">
                    {{ number_format($stillsActual, 2) }}
                </span>
            </p>
            <p>Target:
                <span class="font-bold">
                    {{ number_format($stillsTarget, 2) }}
                </span>
            </p>
            <p>Achievement:
                <span class="font-bold">
                    {{ number_format($stillsAchievement, 2) }}%
                </span>
            </p>
            <p class="font-bold {{ $stillsVariance < 0 ? 'text-red-600' : 'text-green-600' }}">
                Variance:
                {{ $stillsVariance < 0 ? '-' : '+' }}
                {{ number_format(abs($stillsVariance), 2) }}
            </p>
        </div>
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
    ...
    <div class="bg-orange-100 p-4 rounded shadow">
        ...
        <h3 class="text-xl font-bold">
            {{ number_format($actualSales - $targetSales, 2) }}
        </h3>
    </div>
...
<div class="bg-white rounded-xl p-6 shadow mb-6">
    ...
    <div class="flex justify-between items-center mb-3">
        ...
        <span class="text-sm font-semibold px-3 py-1 rounded-full
            ...
            {{ $percent >= 100 ? 'Achieved' : ($percent >= 80 ? 'On Track' : 'Needs Attention') }}
        </span>
    </div>
    ...
    <div class="w-full bg-[#FFF6E5] rounded-full h-4 overflow-hidden">
        <div
            ...
            style="
                width: {{ $percent }}%;
                background-color:
                {{ $percent >= 100 ? '#1f9bb6' : ($percent >= 80 ? '#fbbc05' : '#ff8a00') }};
            "
        ></div>
    </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            <div class="rounded-lg border border-blue-200 bg-blue-50 p-3">
                ...
                <p class="text-sm font-bold text-blue-900">
                    Basis: Period {{ $averagePeriodNo ?? request('period_to', $currentCutoff->period_no ?? $currentCutoff->period_number ?? now()->month) }}
                </p>
                <p class="text-xs text-blue-700 mt-1">
                    Selected Range:
                    P{{ request('period_from', $currentCutoff->period_no ?? $currentCutoff->period_number ?? now()->month) }}
                    to
                    P{{ request('period_to', $currentCutoff->period_no ?? $currentCutoff->period_number ?? now()->month) }}
                </p>
                <p class="text-xs text-blue-700 mt-1">
                    {{ optional($averagePeriodStart)->format('M d, Y') }} to {{ optional($averagePeriodEnd)->format('M d, Y') }}
                </p>
            </div>
            <div class="rounded-lg border border-indigo-200 bg-indigo-50 p-3">
                <p class="text-[11px] text-indigo-800 uppercase font-semibold">Routing Days</p>
                <p class="text-sm font-bold text-indigo-900">{{ number_format($routingDays ?? 0) }} days</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-3">
                <p class="text-[11px] text-gray-700 uppercase font-semibold">Routing Days Left</p>
                <p class="text-sm font-bold text-gray-900">{{ number_format($routingDaysLeft ?? 0) }} days</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
            <div class="rounded-lg border border-orange-200 bg-orange-50 p-3">
                ...
                <p class="text-lg font-bold text-orange-900">&#8369;{{ number_format($averageTargetSales ?? 0, 2) }}</p>
            </div>
            <div class="rounded-lg border border-cyan-200 bg-cyan-50 p-3">
                ...
                <p class="text-lg font-bold text-cyan-900">&#8369;{{ number_format($averageActualSales ?? 0, 2) }}</p>
            </div>
        </div>
        <div style="height:280px;">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-10">
    ...
    {{-- BAR: ACTUAL vs TARGET PER CATEGORY --}}
    <div class="bg-white rounded-xl p-6 shadow">
        <h4 class="font-semibold text-[#002b3d] mb-3">
 ...
 <details class="mt-10 bg-white rounded-xl shadow p-6 border border-[#8ECAE6]">
    ...
    <div class="mt-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="border border-[#219EBC]/40 rounded-lg p-4 bg-[#8ECAE6]/20">
                ...
                <p class="text-sm font-bold text-blue-700 mb-2">
                    {{ number_format($totalReceiptGrossSales ?? 0, 2) }}
                </p>

                <p class="text-xs text-gray-500 mb-2">Total Remittance</p>

                <p class="text-lg font-bold text-blue-700">
                    {{ number_format($totalReceipts ?? 0, 2) }}
                </p>
            ...
            <div class="border border-[#219EBC] rounded-lg p-4 bg-[#8ECAE6]/30">
                ...
                <p class="text-lg font-bold text-green-700">
                    {{ number_format($totalRemittance ?? 0, 2) }}
                </p>
            ...
            <div class="border rounded-lg p-4">
                ...
                <div class="mt-2 text-xs text-gray-600 space-y-1">
                    <div>Account Receivables: <span class="font-semibold">{{ number_format($totalAccountReceivables ?? 0, 2) }}</span></div>
                    <div>Receivable Collections: <span class="font-semibold">{{ number_format($totalReceivableCollections ?? 0, 2) }}</span></div>
                    <div>Stock Transfer: <span class="font-semibold">{{ number_format($totalStockTransfers ?? 0, 2) }}</span></div>
                    <div>Shortage Collections: <span class="font-semibold">{{ number_format($totalShortageCollections ?? 0, 2) }}</span></div>
                </div>
            ...
            <div class="border rounded-lg p-4">
                ...
                <p class="text-lg font-bold">
                    {{ number_format($netBorrowed ?? 0) }}
                </p>

                <div class="mt-2 text-xs text-gray-600 space-y-1">
                    <div>Total Borrowed: <span class="font-semibold">{{ number_format($totalBorrowed ?? 0) }}</span></div>
                    <div>Total Returned: <span class="font-semibold">{{ number_format($totalReturned ?? 0) }}</span></div>
                </div>
        ...
        <div class="flex justify-end">
            <a href="{{ route('admin.reports.consolidated') }}"
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Actual Sales',
                    data: {{ Js::from($chartActual) }},
                    borderWidth: 2
                },
                {
                    label: 'Target Sales',
                    data: {{ Js::from($chartTarget) }},
                    borderWidth: 2
                }
            ]
        },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (lineCtx) {
        const routingDays = Number({{ (int) ($routingDays ?? 0) }});
        const averagePeriodNo = Number({{ (int) ($averagePeriodNo ?? request('period_to', 0)) }});
                ...
                datasets: [
                    {
                        ...
                        data: [
                            {{ (float) ($averageTargetSales ?? 0) }},
                            {{ (float) ($averageActualSales ?? 0) }}
                        ],
    ...
    if (pieCtx) {
        new Chart(pieCtx, {
            type: 'pie',
            data: {
                labels: {{ Js::from($branchPieLabels ?? []) }},
                datasets: [{
                    data: {{ Js::from($branchPieValues ?? []) }},
