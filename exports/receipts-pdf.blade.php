...
<body>
    ...
    <div class="pdf-header">
        ...
        <div class="pdf-meta">
            ...
            <div>Report: Receipts | Date Range: {{ $data['meta']['start'] ?? '—' }} to {{ $data['meta']['end'] ?? '—' }}</div>
            ...
            <tbody>
                @foreach($data['records'] ?? [] as $r)
                    <tr>
                        ...
                        <td>{{ $r->fc ?? '' }}</td>
                        <td>{{ $r->hc ?? '' }}</td>
                        <td class="numeric">{{ $r->box ?? 0 }}</td>
                        <td class="numeric">{{ number_format($r->total_cases ?? 0,2) }}</td>
                        <td class="numeric">{{ number_format($r->total_ucs ?? 0,2) }}</td>
                        <td class="numeric">{{ $r->number_of_receipts ?? 0 }}</td>
                        <td class="numeric">{{ $r->customer_counts ?? 0 }}</td>
                    </tr>
                @endforeach

                {{-- fill blanks so layout looks like sample (rows will be visible but empty) --}}
                @for ($i = 0; $i < max(0, 12 - count($data['records'] ?? [])); $i++)
                    <tr>
                        @for($c=0;$c<9;$c++)<td>&nbsp;</td>@endfor
                    </tr>
                @endfor
    ...
    <div class="outer-box">
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td class="numeric">{{ number_format($data['totals']['gross_sales'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['sales_discounts'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['coupon_discounts'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['net_sales'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['containers_deposit'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['purchased_refund'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['stock_transfer'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['net_credit_sales'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['shortage_collections'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['ar_collections'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['other_income'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['cash_proceeds'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['remit_cash'] ?? 0,2) }}</td>
                    <td class="numeric">{{ number_format($data['totals']['remit_check'] ?? 0,2) }}</td>
                ...
                {{-- Add a few empty rows to match aesthetic --}}
                @for($r=0;$r<6;$r++)
                    <tr>@for($c=0;$c<16;$c++)<td>&nbsp;</td>@endfor</tr>
                @endfor
