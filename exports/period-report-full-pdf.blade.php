...
@foreach($groupedReports as $groupKey => $groupReports)
@php
...
<div class="excel-header">
    <div class="excel-header-top">
        <div class="excel-header-left">
            <div class="branch-name">{{ strtoupper((string) ($report->branch ?? '-')) }}</div>
            <div class="period-name">PERIOD {{ $report->period_no }}</div>
            <div class="date-range">{{ $headerDateRange }}</div>
            <div class="routing-days">
                ({{ $routingDaysCount !== null ? $routingDaysCount . ' routing days' : 'routing days n/a' }})
            </div>
        </div>
        <div class="excel-header-right">
            <table class="excel-summary">
                <thead>
                    <tr>
                        <th class="period-cell">P{{ $report->period_no }}</th>
                        <th>TARGET</th>
                        <th>ACTUAL</th>
                        <th>%</th>
                        <th>VARIANCE</th>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{ number_format($coreRowTarget, 0) }}</td>
                        <td>{{ number_format($coreRowActual, 0) }}</td>
                        <td>{{ number_format($pct($coreRowActual, $coreRowTarget), 0) }}%</td>
                        <td>{{ number_format($var($coreRowActual, $coreRowTarget), 0) }}</td>
                    </tr>
                    <tr class="summary-stills">
                        <td class="label">STILLS</td>
                        <td>{{ number_format($stillsT, 0) }}</td>
                        <td>{{ number_format($stillsA, 0) }}</td>
                        <td>{{ number_format($pct($stillsA, $stillsT), 0) }}%</td>
                        <td>{{ number_format($var($stillsA, $stillsT), 0) }}</td>
                    </tr>
                    <tr class="summary-petcsd">
                        <td class="label">PET CSD</td>
                        <td>{{ number_format($petT, 0) }}</td>
                        <td>{{ number_format($petA, 0) }}</td>
                        <td>{{ number_format($pct($petA, $petT), 0) }}%</td>
                        <td>{{ number_format($var($petA, $petT), 0) }}</td>
                    </tr>
                    <tr>
                        <td class="label total-label">TOTAL</td>
                        <td>{{ number_format($targetTotal, 0) }}</td>
                        <td>{{ number_format($totalActual, 0) }}</td>
                        <td>{{ number_format($pct($totalActual, $targetTotal), 0) }}%</td>
                        <td>{{ number_format($var($totalActual, $targetTotal), 0) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<table class="products-table">
    <thead>
        <tr>
            ...
            <th class="matrix-top-value" colspan="7">{{ $headerDateText }}</th>
        </tr>
        <tr>
            <th class="matrix-top-label" colspan="2">SHIPMENT NO.</th>
            <th class="matrix-top-value" colspan="7">{{ $headerShipmentText }}</th>
        </tr>
        <tr>
            <th colspan="2"></th>
    ...
    <tbody>
        @forelse($orderedCategoryKeys as $categoryKey)
            @php
            ...
            @endphp
            <tr class="category-divider {{ $categoryKey }}">
                <td colspan="9">{{ $categoryLabel }}</td>
            </tr>
            @foreach($categoryPackGroups as $pack => $packRows)
                @foreach($packRows as $rowIndex => $productRow)
                    @php
                    ...
                    <tr>
                        @if($rowIndex === 0)
                            <td class="matrix-pack" rowspan="{{ $packRows->count() }}">{{ $pack }}</td>
                        @endif
                        <td class="matrix-product {{ $productRow['category'] ?? '' }}">{{ $productRow['product'] }}</td>
                        <td class="cell-core-pcs">{{ abs($rowCorePcs) > 0.000001 ? number_format($rowCorePcs, 0) : '0' }}</td>
                        <td class="cell-core-ucs">{{ abs($rowUcs) > 0.000001 ? number_format($rowUcs, 6) : '' }}</td>
                        <td class="cell-core-total">{{ abs($rowCore) > 0.000001 ? number_format($rowCore, 2) : '0.00' }}</td>
                        <td class="cell-iws-cases">{{ abs($rowIwsCases) > 0.000001 ? number_format($rowIwsCases, 0) : '0' }}</td>
                        <td class="cell-iws-total">{{ abs($rowIws) > 0.000001 ? number_format($rowIws, 2) : '0.00' }}</td>
                        <td class="cell-srp">{{ abs($rowSrp) > 0.000001 ? number_format($rowSrp, 2) : '0.00' }}</td>
                        <td class="cell-total-amount">{{ abs($rowTotalAmount) > 0.000001 ? number_format($rowTotalAmount, 2) : '0.00' }}</td>
                    </tr>
                @endforeach
            @endforeach
        @empty
        ...
        @if($productRows->isNotEmpty())
            <tr class="overall-total">
                ...
                <td class="cell-core-pcs">{{ number_format($overallCorePcs, 0) }}</td>
                <td class="cell-core-ucs">-</td>
                <td class="cell-core-total">{{ number_format($overallCoreTotal, 2) }}</td>
                <td class="cell-iws-cases">{{ number_format($overallIwsCases, 0) }}</td>
                <td class="cell-iws-total">{{ number_format($overallIwsTotal, 2) }}</td>
                <td class="cell-srp">-</td>
                <td class="cell-total-amount">{{ number_format($overallTotalAmount, 2) }}</td>
            </tr>
        @endif
    </tbody>
</table>
...
<table class="inv-table">
    ...
    <tbody>
        @foreach($perSkuRows as $row)
            ...
            @endphp
            <tr>
                <td>{{ ($row['product'] ?? '') }} {{ ($row['pack'] ?? '') }}</td>
                <td>{{ number_format($targetPcs, 0) }}</td>
                <td>{{ number_format($targetUcs, 6) }}</td>
                <td>{{ number_format($actualPcs, 0) }}</td>
                <td>{{ number_format($actualUcs, 6) }}</td>
                <td>{{ number_format($varPcs, 0) }}</td>
                <td>{{ number_format($varUcs, 6) }}</td>
            </tr>
        @endforeach
    </tbody>
</table>
...
@foreach($groupReports as $shipmentReport)
    @php
    ...
    <div class="section">{{ $tbl['title'] }} (Shipment {{ $shipmentReport->shipment_no ?: '-' }})</div>
    <table>
    <thead>
    <tr>
