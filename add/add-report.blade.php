<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight flex items-center gap-2">
            ...
            Add Report — {{ $division->division_name }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white shadow p-8 rounded-xl">
                ...
                <div class="flex items-center justify-between mb-4">
                    <div class="text-sm text-black-600">
                        ...
                        <span class="font-bold text-gray-900 capitalize">{{ $reportType }}</span>
                    </div>
                </div>

               {{-- FORM --}}
<form action="{{ route('admin.reports.store', $reportType) }}" method="POST" class="space-y-6">
@csrf
    <div x-data>
...
{{-- ======================================================
     RECEIPTS BREAKDOWN (CLEAN – TOGGLE SAFE)
====================================================== --}}
...
#receiptsTable button {
    ...
    font-size: 16px;
      ...
      {{-- =====================================================
        REMITTANCE DETAILS (RELATIONAL READY)
===================================================== --}}
<div x-data x-show="'{{ $reportType }}' === 'remittance'">
...
<div>
    ...
    <table class="w-full text-sm border">
        ...
        <tr data-row="{{ $i }}">
            ...
            {{-- BANK + BRANCH --}}
            <td class="border p-1">
                @php $bankListId = "banksList_$i"; @endphp

                <input
                    list="{{ $bankListId }}"
                    name="checks[{{ $i }}][bank_branch]"
                ...
                <datalist id="{{ $bankListId }}">
                    @foreach($bankAccounts as $account)
                        <option value="{{ $account->bank_name }} - {{ $account->branch_name }}">
                    @endforeach
                </datalist>
            </td>
            ...
            <td class="border p-1">
                <select
                    ...
                    name="checks[{{ $i }}][account_holder_name]"
                >
            ...
            <td class="border p-1">
                <input type="date"
                       name="checks[{{ $i }}][check_date]"
                       class="w-full border-gray-300 rounded px-1 py-0.5">
            </td>

            <td class="border p-1">
                <input type="text"
                       name="checks[{{ $i }}][remarks]"
                       class="w-full border-gray-300 rounded px-1 py-0.5">
            </td>

            <td class="border p-1">
                <input type="number" step="0.01"
                       name="checks[{{ $i }}][amount]"
...
{{-- ===============================
     MASTER DATA (FROM BANK MANAGEMENT)
=============================== --}}

<select id="all-account-names" class="hidden">
    @foreach ($bankAccounts as $acc)
        <option
            data-bank="{{ trim($acc->bank_name) }}"
            data-branch="{{ trim($acc->branch_name) }}"
...
<select id="all-account-numbers" class="hidden">
    @foreach ($bankAccounts as $acc)
        <option
            data-bank="{{ trim($acc->bank_name) }}"
            data-branch="{{ trim($acc->branch_name) }}"
            data-name="{{ trim($acc->account_holder_name) }}"
    ...
    numberSelect.value = '';
            ...
            <div>
                ...
                <table class="w-full border cash-table">
                    ...
                    <tbody>
                        @php $denoms = [1000,500,200,100,50,20,10,5,1,0.25]; @endphp
                        @foreach ($denoms as $i => $den)
                        <tr>
...
{{-- =====================================================
    RECEIVABLES DETAILS (RELATIONAL – FINAL)
===================================================== --}}
                                ...
                                <select name="ar_customer_id[]" class="customer-select border px-2 py-1 w-full">
                                    ...
                                    @foreach($customers as $customer)
                                        <option
                                            ...
                                            data-remaining="{{ $customer->remaining_credit }}"
                                        >
                                            {{ $customer->store_name }}
                                        </option>
                                    @endforeach
...
{{-- =====================================================
     BORROWER'S AGREEMENT MONITORING (matches actual form)
===================================================== --}}
<div 
    x-data="{
        cols: {
    ...
    class="{{ $reportType !== 'borrowers' ? 'hidden' : '' }}"
>
    ...
    <div class="overflow-x-auto">
        ...
        <table class="w-full text-sm border-collapse">
            ...
            <tbody id="borrowers-body">
                @php $items = ['Plastic','Kasalo','Litro']; @endphp
                @foreach ($items as $item)
                <tr class="borrower-row">
                    <td class="border p-2 font-semibold item-name">{{ $item }}</td>

                    <td x-show="cols.bodega" class="border p-1">
                        <input type="number" name="borrowed_bodega_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>
                    <td x-show="cols.bodega" class="border p-1">
                        <input type="number" name="returned_bodega_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>

                    <td x-show="cols.crs1" class="border p-1">
                        <input type="number" name="borrowed_crs1_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>
                    <td x-show="cols.crs1" class="border p-1">
                        <input type="number" name="returned_crs1_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>

                    <td x-show="cols.crs2" class="border p-1">
                        <input type="number" name="borrowed_crs2_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>
                    <td x-show="cols.crs2" class="border p-1">
                        <input type="number" name="returned_crs2_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>

                    <td x-show="cols.crs3" class="border p-1">
                        <input type="number" name="borrowed_crs3_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>
                    <td x-show="cols.crs3" class="border p-1">
                        <input type="number" name="returned_crs3_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>

                    <td x-show="cols.outside" class="border p-1">
                        <input type="number" name="borrowed_outside_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>
                    <td x-show="cols.outside" class="border p-1">
                        <input type="number" name="returned_outside_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>

                    <td x-show="cols.water" class="border p-1">
                        <input type="number" name="borrowed_water_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>
                    <td x-show="cols.water" class="border p-1">
                        <input type="number" name="returned_water_{{ strtolower($item) }}[]" class="w-full px-1 py-0.5">
                    </td>
                </tr>
                @endforeach
            </tbody>
     ...
     {{-- =====================================================
        GLOBAL ACTION BUTTONS (ALL REPORT TYPES)
    ===================================================== --}}
...
updateFooterTotals();
   ...
   {{-- ======================================================
     AUTO-CALCULATIONS + PESO FORMAT (FIXED)
   ====================================================== --}}
<script>
/*
